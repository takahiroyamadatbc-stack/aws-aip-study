...
## データレジデンシー要件の粒度（「EU域内」と「特定の1リージョン」は違う）

ここを混同すると「CRIで別リージョンに飛ばして本当に法令違反にならないのか？」という不安が拭えなくなる。

- GDPRなどのデータレジデンシー要件が実際に規定しているのは、通常**「特定の法域（EU/EEAなど）の域外にデータを出さないこと」**であり、「AWSの特定の1リージョンに固定すること」ではない。法域とAWSリージョンは粒度が異なる（1つの法域の中に複数のAWSリージョンが存在する）
- 一方で、企業が実装するSCPやガバナンス設定は、法的要件を満たすための**運用上の実装**であり、しばしば「使うリージョンを1つだけに絞る」という、法的要件よりも狭い（保守的な）形になっていることがある。これは「たまたまその1リージョンしか使っていなかったから、Denyの許可リストもそこだけになっていた」というだけで、法律が1リージョン限定を要求しているわけではない
- Geographic CRIの`eu.`プレフィックス推論プロファイルは、**ルーティング先がEUのAWSリージョン（eu-central-1, eu-west-1, eu-west-3, eu-north-1など）に限定される**ことをAWS側が保証する機能。geo境界（この場合EU）を越えたルーティングは発生しない
- したがって、eu-central-1限定のSCPをeu-west-3等のEU域内リージョンにも広げても、データはEU域外に一切出ないため、法的なデータレジデンシー要件（EU域内保持）には抵触しない。変わるのは「運用上のリージョン許可範囲」だけで、「法域の境界」は変わらない
- 逆に、もし本当に「eu-central-1という1リージョンだけを使わなければならない」という契約上・法律上の要求が別途ある場合（EU全域では不十分なケース）は、そもそもCRIを使うこと自体が要件と両立しない。CRIは複数リージョンへのルーティングが前提の機能なので、「1リージョン固定」の要件がある時点でCRI自体を採用すべきではない、という判断になる
...
- 「eu-central-1限定のSCPをeu-west-3等の別EUリージョンにも広げるのはデータレジデンシー要件（EU域内保持）違反になる」→ 誤り。Geographic CRIはgeo境界（EU）を越えてルーティングしないため、EU域内で完結する限り法的要件には抵触しない。「特定の1リージョン限定」は運用上の実装であって法的要件そのものではない点に注意
