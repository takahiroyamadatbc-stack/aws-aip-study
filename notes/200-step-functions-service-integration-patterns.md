...
| 想定用途 | 長時間実行・監査要件のあるワークフロー（本問のような反復ループ＋事後監査） | 秒間数万〜10万超の高頻度・短時間（最大5分）のイベント処理 |
| 料金モデル | **ステート遷移の回数**あたりの課金 | **実行回数＋実行時間＋メモリ**によるLambda的な従量課金 |

**Expressは「上位・高コストな版」ではない**。むしろ大量実行を安価にさばくための料金体系であり、その代わりに実行履歴のネイティブ保持（サービス内蔵のログ機能）を持たない、というトレードオフになっている。実行頻度が低くステート遷移も少ないワークロードをExpressで組んでCloudWatch Logsに転送し続けると、かえってコスト・運用の手間が増えることもある。
