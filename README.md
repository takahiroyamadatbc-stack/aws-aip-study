...
| [004-multi-agent-orchestration](labs/004-multi-agent-orchestration/) | 問67 | マルチエージェントオーケストレーション（Strands Agents SDK vs Step Functions自前実装） | Python/CDK | 検証中 | Strands Agents SDKのWorkflowパターンは推論ループ・ツール呼び出し・コンテキスト受け渡しをSDKが肩代わりするため、同一要件のStep Functions+Lambda自前実装よりコード量が明確に少ない（実測で約2.7倍の差） |
