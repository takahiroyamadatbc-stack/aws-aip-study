...
#### Semantic chunkingの詳細パラメータ

Fixed-size chunkingが「トークン数」という機械的な基準で切るのに対し、Semantic chunkingは**文同士の意味的な距離（Embeddingベクトルのコサイン距離）**を基準に切る。仕組みは以下の3パラメータで制御する。

| パラメータ | 何を制御するか | 挙動 |
|---|---|---|
| ブレークポイントパーセンタイルしきい値（breakpoint percentile threshold） | どこを分割点とみなすか | 文と文の間の意味的距離を全て計算し、その分布の中で指定パーセンタイルを超える（＝周囲と比べて特に意味が大きく変化している）箇所だけを分割点にする。**値を高くする（例: 95%）ほど、よほど意味が変わる箇所でしか分割しない＝チャンクが大きく・少なくなる。**低くするほど細かく分割される |
| バッファサイズ（buffer size） | 分割判定の際、対象の文の前後何文を一緒にEmbedding化するか | `buffer size = 1`なら、判定対象の文の前後1文ずつを加えた**合計3文**をまとめてEmbedding化し、周辺文脈を踏まえた上で意味の変化を判定する。1文だけで判定するより、局所的な言い回しのブレに左右されにくくなる |
| 最大トークン数（max tokens） | 1チャンクが超えてはいけない上限 | 意味的な分割点が見つからず1チャンクが際限なく大きくなるのを防ぐための**安全弁**。ブレークポイントしきい値・バッファサイズによる意味ベースの分割判定より優先され、この上限に達した時点で強制的にチャンクを区切る。Fixed-size chunkingのチャンクサイズと役割は同じだが、Semantic chunkingでは「基本は意味で切る、切れなければトークン数で頭打ちにする」という補助的な位置づけになる点が異なる |

この3つを組み合わせることで、「概念やセクションの境界のような意味的にまとまった単位」でチャンクを作れる点が、Fixed-size/Hierarchical chunking（あくまでトークン数基準）との根本的な違い。技術文書のように論理的なセクション構造を持つドキュメントでは、トークン数基準の分割は文の途中や概念の途中で機械的に切れてしまう可能性があるのに対し、Semantic chunkingは意味の連続性を優先して分割点を決める。
